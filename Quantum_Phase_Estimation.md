...
![image](https://github.com/user-attachments/assets/841669a8-a579-4460-b793-e9a7994f780d)
...
# Define the Hamiltonian for a simple system (e.g., Pauli-Z operator)
H = 0.5 * np.array([[1, 0], [0, -1]])

# Number of qubits for the system and the ancilla qubits for phase estimation
n_system_qubits = 1
n_ancilla_qubits = 3  # Number of ancilla qubits determines the precision

# Initialize the quantum device
dev = qml.device("default.qubit", wires=n_system_qubits + n_ancilla_qubits)

# Define the unitary operator U = exp(-i * H * t)
def unitary_operator(t):
    return expm(-1j * H * t)
...
# Extract the phase (eigenvalue)
phase = np.argmax(probs) / (2 ** n_ancilla_qubits)
eigenvalue = 2 * np.pi * phase

print(f"Estimated phase: {phase}")
print(f"Estimated eigenvalue: {eigenvalue}")

# Plot the probability distribution
import matplotlib.pyplot as plt

plt.bar(range(2 ** n_ancilla_qubits), probs)
plt.xlabel("State")
